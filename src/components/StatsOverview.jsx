...
import { isFinalizedState } from './CausasTable';

export default function StatsOverview({ causas, selectedFilter, onSelectFilter }) {
  // Count en tramite (causes active in instruction - excluding finalized, paradero, and captura)
  const enTramiteCount = (causas || []).filter(c => {
    return !isFinalizedState(c?.estado, c?.tramite);
  }).length;
